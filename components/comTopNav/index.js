...
Component({
  ...
  attached: function () {
    ...
    let btnPosi = { // 胶囊实际位置，坐标信息不是左上角原点
      height: headerPosi.height*2,
      width: headerPosi.width*2,
      top: (headerPosi.top - statusBarHeight)*2, // 胶囊top - 状态栏高度
      bottom: (headerPosi.bottom - headerPosi.height - statusBarHeight)*2, // 胶囊bottom - 胶囊height - 状态栏height （胶囊实际bottom 为距离导航栏底部的长度）
      right: (app.globalData.systeminfo.windowWidth - headerPosi.right)*2 // 这里不能获取 屏幕宽度，PC端打开小程序会有BUG，要获取窗口高度 - 胶囊right
    }
    let haveBack;
    ...
    this.setData({
      ...
      navbarHeight: headerPosi.bottom*2 + btnPosi.bottom, // 胶囊bottom + 胶囊实际bottom
      navbarBtn: btnPosi,
      cusnavH: cusnavH
    });
    //将实际nav高度传给父类页面
    app.store.setState({
      topNavHeight:headerPosi.bottom*2 + btnPosi.bottom
    })
    this.triggerEvent('commonNavAttr',{
      height: headerPosi.bottom*2 + btnPosi.bottom
    });
  },
  methods: {
    handleGoBack: function () {
      if(this.properties.curPageName=='myLoveCar'){
        wx.reLaunch({
